...
| `inject-latency.sh`/`inject-errors.sh` chaos causes an unexpected pod restart, not just a slow/failing response | Already fixed in current `apps/*/backend/src/chaos.js` (`/healthz` and `/readyz` are exempted from the middleware) -- if you still see this, you're on a stale checkout, pull latest | `/healthz` and `/readyz` are the actual liveness/readiness probes (`apps/<app>/k8s/deployment-backend.yaml`); an older `chaos.js` only exempted `/api/chaos/*`, so injected latency/errors could delay or fail the probe itself and cause kubelet to restart the pod as a side effect |
