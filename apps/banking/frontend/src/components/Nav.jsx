  ...
  return (
    <header
      className={`absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-8 py-6 md:px-16 ${
        dark ? "text-white" : "text-meridian-navy"
      }`}
    >
      ...
      <nav className="hidden gap-8 text-xs font-semibold uppercase tracking-[0.2em] md:flex">
        {authed ? (
          <>
            <Link to="/dashboard" className="opacity-80 transition-opacity hover:opacity-60">
              Dashboard
            </Link>
            <Link to="/transfer" className="opacity-80 transition-opacity hover:opacity-60">
              Transfer
            </Link>
